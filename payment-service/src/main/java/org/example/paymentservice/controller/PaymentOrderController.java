...
public class PaymentOrderController {
    ...
    public Result<Boolean> paymentSuccess(
        ...
        // 4. 调用order-service更新订单状态
        System.out.println("\n步骤4: 调用order-service更新订单状态...");
        try {
            Result<Boolean> orderResult = orderFeignClient.updateOrderStatusByPayment(
                payment.getOrderNo(), 
                payment.getOrderId()
            );
            
            if (orderResult != null && Boolean.TRUE.equals(orderResult.getData())) {
                System.out.println("✅ 订单状态更新成功 - 订单号: " + payment.getOrderNo());
                System.out.println("==========================================\n");
                return Result.success(true);
            } else {
                String errorMsg = orderResult != null ? orderResult.getMessage() : "返回结果为null";
                System.err.println("❌ 订单状态更新失败: " + errorMsg);
                System.err.println("⚠️ 但支付已成功，请手动处理或通过补偿任务修复");
                System.err.println("==========================================\n");
                return Result.error("支付成功，但订单状态更新失败: " + errorMsg);
            }
        } catch (Exception e) {
            System.err.println("❌ 调用order-service异常: " + e.getMessage());
            e.printStackTrace();
            System.err.println("⚠️ 但支付已成功，请手动处理或通过补偿任务修复");
            System.err.println("==========================================\n");
